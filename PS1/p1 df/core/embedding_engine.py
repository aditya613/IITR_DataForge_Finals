...
class EmbeddingEngine:
    ...
    def ingest_text_file(self, file_path: str, doc_id: str = None) -> int:
        """
        ...
        for chunk in chunks:
            chunk_start = full_text.find(chunk, current_char)
            chunk_end = chunk_start + len(chunk) if chunk_start != -1 else current_char + len(chunk)
    ...
    def ingest_document(self, file_path: str, doc_id: str = None) -> int:
        """
        Ingest a document (PDF or text file) into the vector database.
        ...
        if file_path.lower().endswith('.pdf'):
            return self.ingest_pdf(file_path, doc_id)
        elif file_path.lower().endswith('.txt'):
            return self.ingest_text_file(file_path, doc_id)
        else:
